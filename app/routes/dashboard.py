...
from app.services import document_service # 문서 저장/조회/삭제 기능이 담긴 서비스 로직을 불러옴

from app.models.document_model import Doc # 응답 모델로 사용할 Document 데이터 구조를 불러옴
...
def upload_file(file: UploadFile):  #?? 이부분 API명세서랑 똑같이 해야하는지 집에서체크해보기
    return document_service.save_file(file)  # 업로드된 파일을 document_service에 전달하여 저장하고 결과 반환

# 저장된 문서 목록을 조회하는 API (GET 요청)
@router.get("/documents", response_model=List[Doc])
def get_documents():
    return document_service.get_all_documents() # 모든 문서를 리스트 형태로 반환

# 특정 문서를 다운로드하는 API (GET 요청)
@router.get("/documents/{document_id}/download")
def download_document(document_id: str):
    doc = document_service.get_document_file(document_id)  # 주어진 ID에 해당하는 문서를 서비스에서 가져옴
    if not doc:
...
def delete_document(document_id: str):
    result = document_service.delete_document(document_id)    # 문서를 삭제 시도
    if not result:
